...
rm -f valgrind_out
/usr/bin/valgrind --leak-check=full --show-leak-kinds=all --errors-for-leak-kinds=all --track-origins=yes ./chatty -f DATA/chatty.conf1 >& ./valgrind_out &
pid=$!
...
echo ""
echo "5"
./client -l $1 -k pippo -p -s ./libchatty.a:minni
